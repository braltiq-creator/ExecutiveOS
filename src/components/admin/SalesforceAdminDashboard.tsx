...
import type { SalesforceAdminStatus } from "@/providers/salesforce";
import { SalesforceAdminControls } from "@/components/admin/SalesforceAdminControls";

type SalesforceAdminDashboardProps = {
  status: SalesforceAdminStatus;
  services: string[];
};
...
export function SalesforceAdminDashboard({
  status,
  services,
}: SalesforceAdminDashboardProps) {
  return (
    <div className="space-y-8">
      <PageHeader
        overline="Administration"
        title="Salesforce"
        description="Commercial connection, CDC health, Platform Events, and commercial context observability."
      />

      <SalesforceAdminControls connected={status.tenantStatus === "connected"} />

      <ResponsiveGrid columns={4}>
        <MetricCard
          label="Connection"
          value={status.tenantStatus}
          hint={status.orgId ?? "Not connected"}
        />
        <MetricCard
        ...
        <ResponsiveGrid columns={3}>
          <Card padding="md">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-zinc-900">Organisation</p>
                <p className="mt-1 text-xs leading-5 text-zinc-600">
                  {status.orgId ?? "Awaiting connection"}
          ...
          <Card padding="md">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-zinc-600">
                  {status.cdcStatus.active} active · {status.cdcStatus.gaps} gaps
                  {status.cdcEnabled ? "" : " · disabled"}
                </p>
              </div>
              <StatusBadge
                label={
                  status.cdcStatus.gaps > 0
                    ? "gap"
                    : status.cdcStatus.active > 0
                      ? "active"
                      : "idle"
                }
                variant={
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-zinc-600">
                  ...
                  {status.platformEventsEnabled ? "" : " · disabled"}
                </p>
              </div>
              <StatusBadge
      ...
      <section className="space-y-3">
        ...
        <Card padding="md">
          ...
          <p className="mt-3 text-xs text-zinc-500">
            Sync frequency: {status.syncFrequency} · Rate remaining{" "}
            {status.rateLimits.remaining ?? "—"}
          </p>
        </Card>
      </section>
      ...
      <section className="space-y-3">
        ...
        <ResponsiveGrid columns={4}>
          ...
          />
          <MetricCard label="Open deals" value={status.dataQuality.openDeals} />
