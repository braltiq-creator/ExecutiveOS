/**
 * Bridge commercial analysis → OutcomePortfolio for existing EIE + Council.
 * No new reasoning engines — constructs portfolio signals from evidence.
 */
...
import type { CommercialAnalysis, CommercialInsight } from "./commercial-analysis";
...
function statusFromInsight(insight: CommercialInsight): OutcomeStatus {
...
function healthFromInsight(insight: CommercialInsight): number {
  if (insight.posture === "insufficient_evidence") return 55;
  if (insight.posture === "investigate") return Math.max(35, 90 - insight.confidence / 2);
...
function buildOutcome(
  ...
  return {
    ...
    expectedTrajectory: {
      ...
      horizonLabel: "This commercial cycle",
    },
    decisionIds: [decisionId],
    contributingInsights: [
      {
        id: `ins-${insight.id}`,
        sourceLabel: "Commercial Snapshot",
        ...
        whatShouldHappenNext:
          insight.posture === "insufficient_evidence"
            ...
            : insight.posture === "monitor"
              ...
              : "Investigate with Council and decide whether to act.",
        recommendation: {
        ...
        ? [
            {
              ...
              description: insight.detail,
              severity:
                insight.posture === "act" ? "critical" : "attention",
              owner: "CRO",
    ...
    stakeholders: [
      {
        ...
        role: "Commercial",
        stance: "sponsor",
        note: "Primary commercial judgement",
      },
      {
        id: "sh-cfo",
        name: "CFO",
        role: "Finance",
        stance: "approver",
        note: "Capital and forecast confidence",
      },
    ],
...
function buildIntent(analysis: CommercialAnalysis): ExecutiveIntent {
  const focus = analysis.insights
    .filter((i) => i.category === "executive_judgement")
    .slice(0, 3)
    .map((i) => `outcome-${i.id.replace(/^judge-/, "")}`);

  return {
    id: "intent-commercial-snapshot",
    title: "Protect commercial forecast clarity",
    narrative:
      "Interpret the commercial book through evidence — not CRM administration. Raise confidence where coverage is weak; force judgement only where evidence supports it.",
    priority: "high",
    ...
    successSignals: [
      {
        ...
        narrative: "Open book is interpretable with transparent confidence.",
      },
    ],
    status: "active",
    history: [],
...
/**
 * Convert commercial analysis into an OutcomePortfolio for existing engines.
 */
export function portfolioFromCommercialAnalysis(
  ...
  const overallScore = Math.max(
    35,
    Math.min(88, 78 - riskWeight * 6 + Math.round(analysis.openCount === 0 ? -5 : 0)),
  );
  ...
  return {
    ...
    statusLabel:
      riskWeight > 0
        ? "Commercial attention required — evidence-based judgements prepared"
        : "Commercial book stable enough to monitor",
